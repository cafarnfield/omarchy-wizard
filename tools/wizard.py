...
MAKEBED_FRAMES = [
    ('makebed0', dict(step=0)),
    ('makebed1', dict(step=1)),
    ('makebed2', dict(step=2)),
    ('makebed3', dict(step=3)),
]
...
def makebed(step=0):
    """Unrolling the bedroll, in four beats.

    He is drawn by `wizard()` with a growing `bob`: that parameter lowers his
    head while the robe stays pinned to the ground, which shortens him into a
    stoop for free rather than needing a whole crouching pose drawn by hand.

    He is also shifted left, because his hem is nearly as wide as the grid and
    would otherwise be painted straight over the mat he is laying out.
    """
    g = blank()
    base = 28
    # He drifts left as he stoops, rather than jumping aside in one frame:
    # starting shifted would pop him half a body-width the moment he began.
    shift = [0, -3, -5, -7][step]

    stoop = [0, 3, 6, 9][step]
    w = wizard(bob=stoop, hem=step % 4)
    for y in range(H):
        for x in range(W):
            if w[y][x] != '.':
                px(g, x + shift, y, w[y][x])

    # The mat, unrolling away from him to the right.
    if step == 0:
        rect(g, 21, base - 3, 25, base, 'D')          # still a bundle
        rect(g, 21, base - 3, 25, base - 3, 'L')
        px(g, 23, base - 1, 'B')
    else:
        reach = {1: 17, 2: 22, 3: 24}[step]
        rect(g, 11, base - 1, reach, base, 'D')
        rect(g, 11, base - 1, reach, base - 1, 'P')
        if step >= 2:
            rect(g, reach - 3, base - 4, reach, base - 1, 'B')
            px(g, reach - 3, base - 4, 'T')
        if step == 1:
            rect(g, reach, base - 3, reach + 3, base, 'D')
            rect(g, reach, base - 3, reach + 3, base - 3, 'L')
...
def emit_js(path):
    order = ([n for n, _ in FRAMES] + [n for n, _ in BOAT_FRAMES]
             ...
             + [n for n, _ in BED_FRAMES] + [n for n, _ in MAKEBED_FRAMES])
    out = []
    ...
    for name, kw in (FRAMES + BOAT_FRAMES + PORTAL_FRAMES + DEATH_FRAMES
                     + WATER_FRAMES + SWIM_FRAMES + BED_FRAMES + MAKEBED_FRAMES):
    ...
    out.append("var MAKEBED = %s" % str([n for n, _ in MAKEBED_FRAMES]).replace("'", '"'))
